...
namespace ORB {
    ...
    ORBFeature::ORBFeature(const std::string &image_one_path, const std::string &image_two_path,
        ...
        orb_extractor_ptr = std::make_shared<ORBextractor>(camera_ptr->nFeatures, camera_ptr->scalseFactor,
                                                           camera_ptr->nLevels, camera_ptr->iniThFAST,
                                                           camera_ptr->minThFAST);
    }
     ...
     */
    void ORBFeature::PoseEstimation(cv::Mat &R21, cv::Mat &t21, std::vector<cv::Point2f> &vPoint_one,
                                    std::vector<cv::Point2f> &vPoint_two) {
        std::vector<cv::KeyPoint> keypoints_1, keypoints_2;
        cv::Mat descriptors_1, descriptors_2;
        std::vector<cv::DMatch> matches, good_matches;
        FindFeatureMatches(image_one, image_two, keypoints_1, keypoints_2, matches, good_matches);
    ...
    void ORBFeature::UndistortImage(const cv::Mat &image, cv::Mat &outImage) {
        ...
        for (int v = 0; v < rows; v++) {
            for (int u = 0; u < cols; u++) {
                ...
                double x_dis = x * (1 + camera_ptr->k1 * r2 + camera_ptr->k2 * r4) + 2 * camera_ptr->p1 * x * y +
                               camera_ptr->p2 * (r2 + 2 * x * x);
                double y_dis = y * (1 + camera_ptr->k1 * r2 + camera_ptr->k2 * r4) + camera_ptr->p1 * (r2 + 2 * y * y) +
                               2 * camera_ptr->p2 * x * y;
    ...
    void ORBFeature::ORBSLAM2ExtractORB(const cv::Mat &srcImage, std::vector<cv::KeyPoint> &vkeypoints,
                                        cv::Mat &descriptors) {
